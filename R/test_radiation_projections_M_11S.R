...
#output folder
out.dir <- ("D:/BP_Layers/M_11S/3PG_flt/1_clipped_rad/")
...
test1 <- rast("D:/BP_Layers/M_11S/3PG_flt/1_clipped_rad/Rad12.tif")
...
terra::writeRaster(tree.age.proj, "D:/BP_Layers/M_11S/inputs/Forest_Age_2019_M_11S.tif", overwrite = T)

# We want to make sure tree age is aligned for relevant study areas
tree.age <- rast("D:/BP_Layers/M_11S/inputs/Forest_Age_2019_M_11S.tif")
...
out.dir = "D:/BP_Layers/M_11S/3PG_flt/4_30m_inputs_all/"
...
############
# Align any layers to the tree.mask - in the case of M_11S it looks like the files are already aligned!
...
#folder with data
fl <- paste0("D:/climate/Future/M_11S/tif_30m/", scen)
#list of files in folder
input.files <- list.files(fl, pattern = ".tif$", full.names = TRUE)
#output folder
...
#folder with data
fp <- paste0("D:/climate/M_11S/output_tif_30m/", scen)
#list of files in folder
tiffs <- list.files(fp, pattern = ".tif$", full.names = TRUE, recursive = TRUE)

#output folder
output_folder <- paste0("D:/BP_Layers/M_11S/3PG_flt/3_frost/", scen) # check we have a /
...
fp = "D:/BP_Layers/M_11S/3PG_flt/1_clipped_rad"
...
# check alignment ...
check.rast1 <- rast("D:/BP_Layers/M_11S/3PG_flt/2_normalized_rad/Rad02.tif")
#check.rast1 <- rast("D:/BP_Layers/M_11S/3PG_flt/2_climate_align/Tmax02.tif")
plot(check.rast1)
...
#####
# aggregate
#fp = "D:/BP_Layers/M_11S/3PG_flt/3_future_frost/Y5_S3"

fp = "D:/BP_Layers/M_11S/3PG_flt/3_frost"

tiffs = list.files(fp, pattern = ".tif$", full.names = T, recursive = T)

# Create a new folder to save the processed rasters
output_folder <- "D:/BP_Layers/M_11S/3PG_flt/5_90m_inputs_all/"
dir.create(output_folder, showWarnings = FALSE)
...
###
# Compare geom
#check.rast <- rast("D:/BP_Layers/M_11S/3PG_flt/5_90m_inputs_all/Tmin03.tif")
#check.rast2 <- rast("D:/BP_Layers/M_11S/3PG_flt/5_90m_inputs_all/Forest_Age_2019.tif")
...
input_dir <- "D:/BP_Layers/M_11S/3PG_flt/5_90m_inputs_all"

output_dir <- "D:/BP_Layers/M_11S/3PG_flt/6_90m_flt"
...
check.rast3 <- rast("D:/BP_Layers/M_11S/3PG_flt/5_90m_inputs_all/PPT09.tif")
...
#directory <- "D:/BP_Layers/M_11S/3PG_flt/6_90m_flt"
directory <- "D:/BP_Layers/M_11S/3PG_flt/6_90m_flt"
...
check.rast4 <- rast("D:/BP_Layers/M_11S/3PG_flt/2_climate_align/Tmax05.tif")


###
# random final checks

check.raster5 <- rast("D:/BP_Layers/M_11S/3PG_flt/6_90m_flt_future/Y5_S1/Tmin09.flt")
check.raster6 <- rast("D:/BP_Layers/M_11S/3PG_flt/6_90m_flt/Forest_Age_2019.flt")

compareGeom(check.rast4, check.raster6)
...
########### Checking folders of geometries

library(terra)

# Replace 'your_folder_path' with the actual path to your folder containing tif files
folder_path <- "D:/BP_Layers/M_11S/3PG_flt/5_90m_inputs_all/"
tif_files <- list.files(path = folder_path, pattern = "\\.tif$", full.names = TRUE)

# Read the first raster
reference_raster <- rast(tif_files[1])

# Function to compare geometry of two rasters
compareGeometry <- function(raster1, raster2) {
    return(compareGeom(raster1, raster2))
}

# Loop through the rest of the rasters
for (i in 2:length(tif_files)) {
    current_raster <- rast(tif_files[i])

    # Compare geometry with the reference raster
    result <- compareGeometry(reference_raster, current_raster)

    if (!result) {
        cat("Geometry mismatch found in:", tif_files[i], "\n")
        break
    }
}

# If the loop completes without breaking, print a success message
if (i == length(tif_files)) {
    cat("All rasters have the same geometry.\n")
}

####################### Check for NAs

# Replace 'your_raster_path' with the actual path to your raster file
raster_path <- "D:/BP_Layers/M_11S/3PG_flt/6_90m_flt/Tmin05.flt"

# Read the raster
na.raster <- rast(raster_path)

# Check for NA values
has_na <- any(is.na(values(na.raster)))

if (has_na) {
    cat("The raster has NA values.\n")

    # Count the number of pixels with NA values
    count_na <- sum(is.na(values(na.raster)))
    cat("Number of pixels with NA values:", count_na, "\n")
} else {
    cat("No NA values found in the raster.\n")
}
