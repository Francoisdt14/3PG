...
biomass_canada <- project(biomass_canada, crs(group.of.49))

# Crop the masked large raster to the extent of the small raster
biomass_canada_cropped <- crop(basal_canada, group.of.49)
# Write the raster
writeRaster(biomass_canada_cropped, filename = paste0("D:/BP_Layers/outputs/crops/ntems_rasters/","biomass", "_","49",".tif"))
...
plot(basal_3pg, col=viridis(100),  main="3PG")
plot(basal_ntems, col=viridis(100),  main="NTEMS")
...
min_value <- minmax(basal_3pg)[1,]
max_value <- minmax(basal_3pg)[2,]

# Rescale the raster between 0 and 1
basal_3pg_NORM <- (basal_3pg - min_value) / (max_value - min_value)

plot(basal_3pg_NORM, col=viridis(100),  main="3PG Normalized")
